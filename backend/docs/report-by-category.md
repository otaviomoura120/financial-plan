...
- **RPTC3 — no transfers.** `TRANSFER` transactions are excluded entirely (they are already excluded from the sums of `POST /reports`). `type=TRANSFER` therefore yields an empty report. Note transfers *do* now carry a category — the reserved "Transferência" system category (see RPTC10) — but the exclusion is by `type`, not by the absence of a category, so nothing changed here.
- **RPTC10 — system categories never contribute.** Credit card invoice payments and transfers are stamped with reserved system categories (`backend/docs/system-categories.md`). They are still excluded from this report by RPTC2/RPTC3, which stay essential because historical rows were not migrated: pre-existing invoice payments carry a hand-picked real category, and pre-existing transfers carry none. A system category can therefore never appear as a group here — which is why the by-category filter dropdown hides them, while the statement's dropdown keeps them.
