...
`CategoryReportItemResponse`: `{ id, source (TRANSACTION|CREDIT_CARD), type, date, description, amount, userId, bankAccountId, creditCardId, creditCardName, installmentNumber, totalInstallments, totalAmount }`. `id`s can collide across the two sources — consumers must key on `source + id`. For `CREDIT_CARD` items, `type` is always `EXPENSE`; the item still enters/leaves the report based on its `referenceMonth` (RPTC1), but `date` keeps showing the original `purchaseDate` as a reference of when the purchase happened, and `bankAccountId` comes from the card's linked account (nullable); `creditCardId`/`creditCardName` satisfy the "which card" requirement. `totalAmount` is the sum of every installment's `amount` in the group (computed on read via `findByInstallmentGroupId`, not persisted) for `CREDIT_CARD` items — equal to `amount` itself for a single (`totalInstallments <= 1`) card purchase, and the full original purchase total for an installment purchase, shown as a reference alongside the per-installment `amount`; it's `null` for `TRANSACTION` items.
