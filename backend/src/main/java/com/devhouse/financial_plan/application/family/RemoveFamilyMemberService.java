...
public class RemoveFamilyMemberService {
    ...
    public void execute(Long familyId, Long userId) {
        ...
        if (!family.getId().equals(user.getFamilyId())) throw new DomainException("User does not belong to this family");
        if (family.getOwnerId().equals(userId)) throw new DomainException("Cannot remove the family owner");
