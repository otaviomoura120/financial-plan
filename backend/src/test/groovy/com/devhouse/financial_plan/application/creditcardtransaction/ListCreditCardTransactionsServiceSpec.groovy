...
class ListCreditCardTransactionsServiceSpec extends Specification {
    ...
    def "execute returns the credit card transactions matching the filter"() {
        ...
        creditCardTransactionRepository.findByFilter(1L, 10L, null, null, LocalDate.of(2026, 3, 1), LocalDate.of(2026, 3, 31)) >> [transaction]
    ...
    def "execute returns an empty list when there are no matches"() {
        ...
        when:
        List<CreditCardTransactionResponse> responses = service.execute(99L, null, null, null, null, null)
